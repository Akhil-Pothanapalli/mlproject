...
def evaluate_models(X_train, y_train, X_test, y_test, models, param):
    ...
    try:
        ...
        for i in range(len(list(models))): #looping through the models 
            model = list(models.values())[i]     #selecting a model and its parameters 
            para = param[list(models.keys())[i]]

            gs = GridSearchCV(model, para, cv=3) # hyper parameter tuning is done here
            ...
            # train to score for best model selection
            # test score to its rel performance
